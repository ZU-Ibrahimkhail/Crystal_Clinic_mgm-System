...
{
    public class CreatePartnersValidator : AbstractValidator<CreatePartnersCommand>
    {
        public CreatePartnersValidator(IStringLocalizer<CommonValidationResource> localizer, IStringLocalizer<CommonColumnNameResource> ColumnLocalizer, IGeneralHelperRepositoryAsync<ERP_DbContext, Partners> helper)
        {
            LocalizeMessage localizeMessage = new(localizer);


            RuleFor(x => x.Email)
                .Must((x, y) => helper.IsUnique(y, nameof(x.Email)))
            ...
            RuleFor(x => x.Phone).NotEmpty()
                ...
                .Must((x, y) => helper.IsUnique(y, nameof(x.Phone)))
