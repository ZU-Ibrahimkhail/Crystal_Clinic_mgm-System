...
namespace Crystal_Clinic_Mgm.Application.Look.ExpenseTypes.Commands.Update
{
    public class UpdateHandler(
        IGenericRepositoryAsync<ERP_DbContext, ExpenseType> genericRepositoryAsync,
        IStringLocalizer<CommonValidationResource> localizer,
        IGeneralHelperRepositoryAsync<ERP_DbContext, ExpenseType> _helper,
        IMessage message,
        ILoggedInUser loggedInUser) : IRequestHandler<UpdateExpenseTypeCommand, JsonResult>
    {
        ...
        private readonly IGeneralHelperRepositoryAsync<ERP_DbContext, ExpenseType> _helper = _helper;
        private readonly IMessage _message = message;
        private readonly ILoggedInUser _loggedInUser = loggedInUser;

        public async Task<JsonResult> Handle(UpdateExpenseTypeCommand request, CancellationToken cancellationToken)
        {

            var validator = new UpdateExpenseTypeValidator(_Localizer, _helper).Validate(request).Errors;
