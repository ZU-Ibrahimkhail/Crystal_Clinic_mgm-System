...
{
    public class GetPayTypeDDLHandler(IGenericRepositoryAsync<ERP_DbContext, PayType> genericRepositoryAsync) : IRequestHandler<GetPayTypeDDLQuery, List<GetDropDownGeneralModel>>
    {

        private readonly IGenericRepositoryAsync<ERP_DbContext, PayType> _GenericRepositoryAsync = genericRepositoryAsync;

        public async Task<List<GetDropDownGeneralModel>> Handle(GetPayTypeDDLQuery request, CancellationToken cancellationToken)
        {
