...
namespace Crystal_Clinic_Mgm.Application.Look.PayTypes.Commands.Update
{
    public class UpdateHandler(
        ...
        IGeneralHelperRepositoryAsync<ERP_DbContext, PayType> _helper,
        IMessage message,
        ILoggedInUser loggedInUser) : IRequestHandler<UpdatePayTypeCommand, JsonResult>
    {
        ...
        private readonly IGeneralHelperRepositoryAsync<ERP_DbContext, PayType> _helper = _helper;
        private readonly IMessage _message = message;
        private readonly ILoggedInUser _loggedInUser = loggedInUser;

        public async Task<JsonResult> Handle(UpdatePayTypeCommand request, CancellationToken cancellationToken)
        {

            var validator = new UpdatePayTypeValidator(_Localizer, _helper).Validate(request).Errors;
