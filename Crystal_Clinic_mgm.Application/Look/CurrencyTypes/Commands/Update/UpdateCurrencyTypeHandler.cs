...
namespace Crystal_Clinic_Mgm.Application.Look.CurrencyTypes.Commands.Update
{
    public class UpdateHandler(
        ...
        IGeneralHelperRepositoryAsync<ERP_DbContext, CurrencyType> _helper,
        IMessage message,
        ILoggedInUser loggedInUser) : IRequestHandler<UpdateCurrencyTypeCommand, JsonResult>
    {
        ...
        private readonly IGeneralHelperRepositoryAsync<ERP_DbContext, CurrencyType> _helper = _helper;
        private readonly IMessage _message = message;
        private readonly ILoggedInUser _loggedInUser = loggedInUser;

        public async Task<JsonResult> Handle(UpdateCurrencyTypeCommand request, CancellationToken cancellationToken)
        {

            var validator = new UpdateCurrencyTypeValidator(_Localizer, _helper).Validate(request).Errors;
