...
namespace Crystal_Clinic_Mgm.Application.Look.LoanTypes.Commands.Update
{
    public class UpdateHandler(
        IGenericRepositoryAsync<ERP_DbContext, LoanType> genericRepositoryAsync,
        IStringLocalizer<CommonValidationResource> localizer,
        IGeneralHelperRepositoryAsync<ERP_DbContext, LoanType> _helper,
        IMessage message,
        ILoggedInUser loggedInUser) : IRequestHandler<UpdateLoanTypeCommand, JsonResult>
    {
        ...
        private readonly IGeneralHelperRepositoryAsync<ERP_DbContext, LoanType> _helper = _helper;
        private readonly IMessage _message = message;
        private readonly ILoggedInUser _loggedInUser = loggedInUser;

        public async Task<JsonResult> Handle(UpdateLoanTypeCommand request, CancellationToken cancellationToken)
        {

            var validator = new UpdateLoanTypeValidator(_Localizer, _helper).Validate(request).Errors;
