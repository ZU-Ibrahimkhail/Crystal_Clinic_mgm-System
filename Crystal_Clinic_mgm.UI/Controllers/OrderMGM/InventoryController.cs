...
using Crystal_Clinic_Mgm.Application.Inventory.Queries;
...
namespace Crystal_Clinic_Mgm.UI.Controllers.OrderMGM
{
    ...
    public class InventoryController : BaseController
    {
        ...
        {
            query.isRental = false;
            var result = await Mediator.Send(query);
            return Ok(result);
        }

        [HttpGet("rental-items")]
        public async Task<IActionResult> GetRentalItems([FromQuery] GetItemsQuery query)
        {
            query.isRental = true;
            var result = await Mediator.Send(query);
            return Ok(result);
        ...
        [HttpPost("items")]
        public async Task<IActionResult> CreateItem([FromBody] CreateItemWithUnitsCommand command)
        {
            var id = await Mediator.Send(command);
        ...
        [HttpPost("items/add-unit")]
        public async Task<IActionResult> AddUnit([FromBody] AddItemUnitCommand command)
        {
            var id = await Mediator.Send(command);
            return Ok(new { UnitId = id });
        }

        [HttpPut("items/update-unit")]
        public async Task<IActionResult> UpdateUnit([FromBody] UpdateItemUnitCommand command)
        {
            var id = await Mediator.Send(command);
            return Ok(new { UnitId = id });
        }

        [HttpDelete("items/delete-unit/{unitId}")]
        public async Task<IActionResult> DeleteUnit(int unitId)
        {
            var id = await Mediator.Send(new DeleteItemUnitCommand { UnitId = unitId });
            return Ok(new { DeletedUnitId = id });
        }
    }
}
